...
typedef struct {
	int len;
	char payload[MAX_LEN]; // > ETHERNET > ARP / IP
	int interface;
} packet;
...
/** 
 * Creates a new entry in the arp table that consists
 * of a pair of type (ip, mac).
 */
struct arp_entry* create_new_entry(uint32_t ip, uint8_t* mac);

/**
 * Inserts the pair entry in the arp table.
 */
void insert_entry(struct arp_entry *entry, struct arp_entry** arp_table, int *size);

/**
 * When an arp request is recieved, it modifies the ethernet and arp headers,
 * turns the request into a reply and sends it back.
 */
void send_reply(struct arp_header** arp_hdr, struct ether_header** eth_hdr, packet* m);

/**
 * Sends an icmp error, depending on the type recieved.
 */
void send_icmp(packet *m, int type);

/**
 * Parses an ICMP header from a packet.
 */
...
/**
 * Binary search algorithm for finding the leftmost element
 */
